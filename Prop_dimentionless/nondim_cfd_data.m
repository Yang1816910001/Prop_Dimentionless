...
% 读取 CFD_DATA_clean.txt，写出 CFD_DATA_nondim.txt / .mat
    ...
    if isempty(dataFile)
        dataFile = fullfile(here, 'CFD_DATA_clean.txt');
    end
    D = p.Results.D;
    rho = p.Results.rho;
    ...
    outTxt = fullfile(here, 'CFD_DATA_nondim.txt');
    outMat = fullfile(here, 'CFD_DATA_nondim.mat');
    ...
    lutTxt = fullfile(here, 'CFD_DATA_lookup.txt');
    lutMat = fullfile(here, 'CFD_DATA_lookup.mat');
