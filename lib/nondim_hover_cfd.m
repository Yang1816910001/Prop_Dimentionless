...
% 悬停 V=0 四点拟合 C = C0 (1+k M_tip^2)，表内存 C_inc。
    ...
    if isempty(dataFile)
        dataFile = fullfile(io.results, 'CFD_HOVER_clean.txt');
    end
    D = p.Results.D;
    rho = p.Results.rho;
    ...
    outTxt = fullfile(io.results, 'CFD_HOVER_nondim.txt');
    outMat = fullfile(io.results, 'CFD_HOVER_nondim.mat');
    ...
    Tlut.('PROP 1 RPM') = [];
    Tlut.('PROP 5 RPM') = [];
    lutTxt = fullfile(io.results, 'CFD_HOVER_lookup.txt');
    lutMat = fullfile(io.results, 'CFD_HOVER_lookup.mat');
